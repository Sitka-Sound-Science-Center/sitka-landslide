...
Beginning in September 2024 ([see PR #90](https://github.com/azavea/sitka-landslide/pull/90)), the build process also, once it's finished building and publishing the new version of the site, writes a copy of that `rainfall.json` file to an archive bucket on S3.  This provides a historical record of all the forecasts and observed conditions, including their risk ratings based on the model, that have been displayed on the site.
